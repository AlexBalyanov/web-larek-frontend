...
эндпоинт сервера\.
По умолчанию стоит POST метод запроса, который можно изменить, передав третий\
параметр при вызове.
...
**Методы класса:**
- `set basketList(items: IBasketItem[])` - сеттер сохраняет список товаров в соответсвующее поле;
...
- `set price(value: number | string)` - сеттер, устанавливающий цену товара.
