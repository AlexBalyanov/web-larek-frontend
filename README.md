...
**В полях класса хранятся следующие данные:**
- `contentElement: HTMLElement` - разметка, переданная для отрисовки в модальном окне.
...
- `handleEscUp(evt: KeyboardEvent): void` - закрыть модальное окно по кнопке "Esc".
...
- `set content(item: HTMLElement[])` - сеттер сохраняет список товаров в соответсвующее поле;
...
**В полях класса хранятся следующие данные:**
- `form: HTMLFormElement` - ткущая форма;
- `inputs: NodeListOf<HTMLInputElement>` - список полей ввода в текущей форме;
...
**Методы класса:**
- `onInputChange()` - генерирует событие при вводе данных в поле;
...
- `getInputsValues()` - получить все значения из полей ввода.
...
- `addressInputElement: HTMLInputElement` - инпут для ввода адреса доставки;
- `events: IEvents` - экземпляр класса EventEmitter.

**Методы класса:**
- `set isOnlineOrCash(value: boolean)` - сеттер, устанавливающий способ доставки;
...
**В полях класса хранятся следующие данные:**
- `emailInputElement: HTMLInputElement` - поле ввода для электронной почты;
- `phoneInputElement: HTMLInputElement` - поле ввода для номера телефона;
- `events: IEvents` - экземпляр класса EventEmitter.
...
- `set image(value: string)` - сеттер, устанавливающий картинку товара;
- `set buttonDisable(value: boolean)` - сеттер, блокирующий кнопку добавления в корзину;
- `set buttonText(value: string)` - сеттер, устанавливающий текст кнопки.
...
**Методы класса:**
- `set index(value: number)` - сеттер, устанавливающий порядковый номер в корзине.

#### Класс Basket
Дочерний класс `ProductItem`, содержит в себе разметку корзины товаров.\
Конструктор класса принимает `container` - родительский контейнер, в котором будет\
происходить поиск DOM элементов, `events` - экземпляр класса EventEmitter для генерации событий.

**В полях класса хранятся следующие данные:**
- `protected contentElement: HTMLElement` - элемент для хранения содержимого корзины;
- `orderButtonElement: HTMLButtonElement` - кнопка оформления заказа в корзине;
- `totalPriceElement: HTMLElement` - итоговая стоимость товаров в корзине;
- `events: IEvents` - экземпляр класса EventEmitter.

**Методы класса:**
- `set valid(value: boolean)` - сеттер, устанавливающий валидность корзины;
- `set content(items: HTMLElement[])` - сеттер, устанавливающий контент корзины;
- `set totalPrice(value: number)` - сеттер, устанавливающий общую стоимость товаров.
